...
import { getEntities, searchGenericsEntities, reset } from 'app/entities/generics/generics.reducer';
...
const GenericsShower = props => {
  ...
  const genericsList = useAppSelector(state => state.generics.entities);
  const loading = useAppSelector(state => state.generics.loading);
  const totalItems = useAppSelector(state => state.generics.totalItems);

  const getAllEntities = () => {
    if (search) {
      dispatch(
        searchGenericsEntities({
          query: search,
  ...
  return (
    <div className="Container px-md-5 mt-4">
      <h2 id="generics-heading" data-cy="GenericsHeading">
      ...
      <Row>
        {genericsList && genericsList.length > 0 ? (
          genericsList.map((elm, idx) => {
            return (
              <Col md="4" lg="3" sm="12" className="mb-5" key={elm.id}>
                <Link to={`/guest/generics/${elm.id}`}>
                  <Card>
                    {/* <CardImg top height="200px" alt="Type" src={imageType[elm.type]} /> */}
                    <CardBody>
                      <CardTitle tag="h5">{elm.gname}</CardTitle>
                      <CardSubtitle tag="h6" className="mb-4 mt-0 text-muted">
                        {elm.ingredientsused ? elm.ingredientsused.iname : ''}
                      </CardSubtitle>
      ...
      {totalItems ? (
        <div className={genericsList && genericsList.length > 0 ? '' : 'd-none'}>
